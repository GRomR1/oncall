...
import {
  getGrafanaPluginSettings,
  installPlugin,
  checkIfPluginIsConnected,
  syncDataWithOnCall,
  getHumanReadableErrorFromOnCallError,
} from 'state/plugin';
...
export class RootBaseStore {
  ...
  async setupPlugin(meta: OnCallAppPluginMeta) {
    ...
    const pluginConnectionStatus = await checkIfPluginIsConnected(this.onCallApiUrl);
    ...
    if (is_user_anonymous) {
      ...
      try {
        ...
        return this.setupPluginError(getHumanReadableErrorFromOnCallError(e, this.onCallApiUrl, 'install'));
      }
    } else {
      const syncDataResponse = await syncDataWithOnCall(this.onCallApiUrl);
  ...
  async getApiUrlForSettings() {
    const settings = await getGrafanaPluginSettings();
