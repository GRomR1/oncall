...
import { selfHostedInstallPlugin as selfHostedInstallPluginOriginal } from 'state/plugin';
...
const fillOutFormAndTryToSubmit = async (onCallApiUrl: string, selfHostedInstallPluginSuccess = true) => {
  ...
  selfHostedInstallPlugin.mockResolvedValueOnce(
    selfHostedInstallPluginSuccess ? null : SELF_HOSTED_PLUGIN_API_ERROR_MSG
  );
...
describe('ConfigurationForm', () => {
  ...
  test('It calls the onSuccessfulSetup callback on successful form submission', async () => {
    ...
    expect(selfHostedInstallPlugin).toHaveBeenCalledWith(VALID_ONCALL_API_URL);
  ...
  test("It doesn't allow the user to submit if the URL is invalid", async () => {
    ...
    expect(selfHostedInstallPlugin).toHaveBeenCalledTimes(0);
