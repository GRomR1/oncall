...
import { selfHostedInstallPlugin } from 'state/plugin';
...
const ConfigurationForm: FC<Props> = ({ onSuccessfulSetup }) => {
  ...
  const setupPlugin: SubmitHandler<FormProps> = useCallback(async ({ onCallApiUrl }) => {
    ...
    const errorMsg = await selfHostedInstallPlugin(onCallApiUrl);
  ...
  return (
    <Form<FormProps> defaultValues={{ onCallApiUrl: getItem(ONCALL_API_URL_LOCAL_STORAGE_KEY) }} onSubmit={setupPlugin}>
      {({ register, errors }) => (
        <>
