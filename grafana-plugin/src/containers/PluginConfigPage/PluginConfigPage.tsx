...
import {
  checkIfPluginIsConnected,
  PluginStatusResponseBase,
  resetPlugin,
  selfHostedInstallPlugin,
  syncDataWithOnCall,
} from 'state/plugin';
...
const PluginConfigPage: FC<OnCallPluginConfigPageProps> = ({
  ...
  const triggerDataSyncWithOnCall = useCallback(async () => {
    ...
    const syncDataResponse = await syncDataWithOnCall(onCallApiUrl);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!pluginMetaOnCallApiUrl && processEnvOnCallApiUrl) {
         ...
         */
        const errorMsg = await selfHostedInstallPlugin(processEnvOnCallApiUrl);
        if (errorMsg) {
       ...
       */
      if (onCallApiUrl) {
        const pluginConnectionResponse = await checkIfPluginIsConnected(onCallApiUrl);
  ...
  } else if (pluginConnectionCheckError) {
    content = (
      <>
        <StatusMessageBlock text={pluginConnectionCheckError} />
        <RemoveConfigButton />
      </>
    );
  } else if (syncError) {
  ...
  } else if (pluginResetError) {
    content = <StatusMessageBlock text={pluginResetError} />;
