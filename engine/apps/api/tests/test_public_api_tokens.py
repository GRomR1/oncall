...
@pytest.mark.parametrize(
    ...
    [
        ([RBACPermission.Permissions.API_KEYS_READ], status.HTTP_200_OK),
        ([RBACPermission.Permissions.API_KEYS_WRITE], status.HTTP_403_FORBIDDEN),
    ],
)
def test_public_api_tokens_retrieve_permissions(
    ...
    organization, user, plugin_token = make_organization_and_user_with_plugin_token(permissions)
...
@pytest.mark.parametrize(
    "permissions,expected_status",
    [
        ([RBACPermission.Permissions.API_KEYS_WRITE], status.HTTP_201_CREATED),
        ([RBACPermission.Permissions.API_KEYS_READ], status.HTTP_403_FORBIDDEN),
    ],
)
def test_public_api_tokens_create_permissions(
    ...
    _, user, plugin_token = make_organization_and_user_with_plugin_token(permissions)
    client = APIClient()

    url = reverse("api-internal:api_token-list")
    response = client.post(url, data={"name": "helloooo", }, format="json",
...
@pytest.mark.parametrize(
    "permissions,expected_status",
    [
        ([RBACPermission.Permissions.API_KEYS_WRITE], status.HTTP_204_NO_CONTENT),
        ([RBACPermission.Permissions.API_KEYS_READ], status.HTTP_403_FORBIDDEN),
    ],
)
